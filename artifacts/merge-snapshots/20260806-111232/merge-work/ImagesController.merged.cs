...
<<<<<<< artifacts\merge-snapshots\20260806-111232\merge-work\ImagesController.local.cs
        ...
        try
        {
            ...
            using var response = await SendGenerationRequestWithRetry(http, endpoint, effectiveRequest, provider, ct).ConfigureAwait(false);
            var body = await response.Content.ReadAsStringAsync(ct).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
                return StatusCode((int)response.StatusCode, new
                {
||||||| artifacts\merge-snapshots\20260806-111232\merge-work\ImagesController.base.cs
        http.BaseAddress = new Uri(provider.BaseUrl.TrimEnd('/') + "/");
        http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", provider.ApiKey);
...
=======
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
                return StatusCode((int)response.StatusCode, new
                {
>>>>>>> artifacts\merge-snapshots\20260806-111232\merge-work\ImagesController.remote.cs
